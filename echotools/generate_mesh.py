...
def estimate_round_off_buffer(patient, time, echo_path, ply_dir, mesh_type = "lv",
                              use_gamer = False, mesh_char_len = 1.0):
    ...
    if get_round_off_buffer(patient, time):
        ...
        err = create_mesh_w_round_off_buffer(patient, 
                                             time, 
                                             echo_path,
                                             ply_dir,
                                             mesh_type, 
    ...
    if estimate_round_off_buffer:
        ...
        res = minimize(measured_vs_computed_volume, method="bounded", 
                       bounds=(-1.0,1.0), options={"xatol" : 0.01})
      
        round_off_buffer = res.x
        logger.info("Optimization finished. Round of buffer = {}".format(round_off_buffer))
        save_round_off_buffer(round_off_buffer, patient, time)

    return round_off_buffer

def create_mesh_w_round_off_buffer(patient, time, echo_path, ply_dir, 
                                   mesh_type, mesh_char_len, 
                                   round_off_buffer, use_gamer,
                                   return_domain = True):
    ...
    try:
        data = create_lv_ply_files(echo_path, time, round_off_buffer,
                                   True,  ply_dir, use_gamer = use_gamer)
   
        
        # Mesh ply files using Gmsh
        mesh, markers = create_lv_geometry(time, 
                                           data["plydir"], 
                                           mesh_char_len=mesh_char_len, 
...
def create_mesh(patient, time, echo_path, 
                ply_dir=".", mesh_type = "lv", 
                mesh_char_len = 1.0, use_gamer = False, **kwargs):

    # Estimate were we should cut the 
    round_off_buffer = estimate_round_off_buffer(patient, 
                                                 time, 
                                                 echo_path,
                                                 ply_dir, 
                                                 mesh_type,
    ...
    mesh, markers, err, data = create_mesh_w_round_off_buffer(patient, 
                                                              time, 
                                                              echo_path,
                                                              ply_dir, 
                                                              mesh_type, 
...
class MeshConstructor(object):
    ...
    def generate_local_basis_functions(self):
        ...
        if self.parameters["generate_local_basis"]:
            ...
            self.local_basis = generate_local_basis_functions(self._mesh,
                                                              self.surface_data["focal_point"])
        else:
    ...
    def save(self, overwrite_file = True, overwrite_group=True):
        ...
        save_geometry_to_h5(self._mesh, self.parameters["h5name"],"", 
                            markers, self.fields,self.local_basis,
                            overwrite_file=overwrite_file,
                            overwrite_group=overwrite_group)

        ply_folder = os.path.join(self.parameters["ply_dir"],
                                  "_".join([self.parameters["patient"],
                                            self.parameters["mesh_type"]]))
...
def setup_mesh_parameters(echopath = ".", h5name = "test.h5", mesh_char_len = 0.65, time = 0):
    ...
    if mesh_char_len in charlen_to_res.keys():
