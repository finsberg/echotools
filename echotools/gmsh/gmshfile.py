...
class GmshFile(object):
    ...
    def __init__(
        self, ifilename=None, geometric_dimension=None, verbose=False, marker_ids=None
    ):
        """FIXME:"""
        ...
        for d in ["_entities", "_ent_tags"]:
            setattr(
                self,
                d,
                {k: np.array(v, dtype="uintp") for k, v in getattr(self, d).items()},
            )
        ...
        for k, v in self._ent_tags.items():
            ...
            for marker in np.unique(v):
                markers_from_entity[self.marker_mapping[marker]] = (None, dim)
        ...
        for codim in range(4):
            if self.entity_markers(codim) is not None:
    ...
    def _digest_file(self, fhandle):
        ...
        sections = [
            s.split("_")[1] for s in dir(self) if re.match(r"^parse_([a-zA-Z])+$", s)
        ]

        # parse as csv
        csv_file = csv.reader(fhandle, delimiter=" ", skipinitialspace=True)
    ...
    def _parse_MeshFormat(self, l):
        ...
        if self.verbose:
            print("Found supported GMSH header")
    ...
    def _parse_Nodes(self, l):
        ...
        if len(l) == 1:
            ...
            self._nodeids = {}
            if self.verbose:
                print(("Expecting %d nodes" % totnodes))
            return
    ...
    def _parse_Elements(self, l):
        ...
        if len(l) == 1:
            ...
            if self.verbose:
                print(("Expecting %d entities" % totelms))
            return
        ...
        # dofs
        dofs = [self._nodeids[int(dof)] for dof in l[3 + num_tags :]]
    ...
    def _parse_PhysicalNames(self, l):
        ...
        if len(l) == 1:
            if self.verbose:
                print(("Expecting {} physical names".format(l[0])))
    ...
    def _entity_exists(self, codim):
        ...
        if enti_type not in list(self._entities.keys()):
            return False
