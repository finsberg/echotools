...
def mark_lv_strain_regions(fun, mesh, focal_point, strain_regions, strain_type):
    """
...
def make_crl_basis(mesh, foc, space="Quadrature_4"):
    ...
    msg = (
        "Creating local basis function in the "
        "circumferential, radial and longitudinal "
        "direction..."
    )
    logger.info(msg)
    family, degree = space.split("_")
    fem = dolfin.FiniteElement(
        family=family, cell=mesh.ufl_cell(), degree=int(degree), quad_scheme="default"
    )
    vem = dolfin.VectorElement(
        family=family, cell=mesh.ufl_cell(), degree=int(degree), quad_scheme="default"
    )
