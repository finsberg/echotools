...
class EchoFile(object):
    ...
    def strain_faces(self):
        ...
        if not hasattr(self, '_strain_faces'):
            ...
            self._strain_faces = \
                np.array([strain_regions.strain_faces(i) for i in range(1,18)])
    ...
    def strain_verts(self, time, scale=100):
        ...
        """
        return surface.\
            strain_mesh_coordinates(self.strain_mesh(time, scale))

    def trasformation_matrix(self, time):
        return surface.get_geometric_matrix(self.strain_mesh(time),
                                            np.copy(self.endo_verts(time)))

    def list_strain_types(self):
        return set(np.transpose([k.split('_')
                                 for k in self._strain_trace.keys()])[0])

    def list_strain_regions(self, strain_type='RadialStrain'):
        return set(np.transpose([k.split('_')
                                 for k in self._strain_trace.keys()
                                 if k.split('_')[0] == strain_type])[1])
    ...
    def strain_trace(self, strain_type, strain_region):
        ...
        if key not in self._strain_trace:
            msg = ('Wrong strain type {0} or strain region {1}. '
                   '\nPossible strain types {2}'
                   '\nCheck list_strain_regions for possible '
                   'strain regions').format(strain_type,
                                            strain_region,
                                            self.list_strain_types())
            raise KeyError(msg)
