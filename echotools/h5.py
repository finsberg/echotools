...
def dolfin_to_hd5(obj, h5name, time=None, comm=None, name=None):
    ...
    logger.info("Save {0} to {1}:{0}/{2}".format(name, h5name, time))
...
if __name__ == "__main__":
    ...
    scalar_functions[0].vector()[:] = np.linspace(
        0, 1, len(scalar_functions[0].vector())
    )
    vector_functions[0].vector()[:] = np.linspace(
        0, 1, len(vector_functions[0].vector())
    )
